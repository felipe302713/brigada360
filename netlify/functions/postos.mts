...
export default async (req: Request, _context: Context) => {
  ...
  if (req.method === 'GET') {
    if (id) return Response.json(db.postos.find(p => p.id === id) || null);
    return Response.json(db.postos);
  }
  ...
  if (req.method === 'POST') {
    ...
    db.postos.push(item);
    db.auditoria.push({ id: uuid(), usuario_id: auth.id, acao: 'criar', entidade: 'postos', entidade_id: item.id, dados_anteriores: null, dados_novos: { nome: item.nome }, created_at: new Date().toISOString() });
    return Response.json(item, { status: 201 });
  }
  if (req.method === 'PUT' && id) {
    const item = db.postos.find(p => p.id === id);
